...
let github_btn = document.querySelector('.github-toggle');
 github_btn.addEventListener('click',function() {     // Initail button for showing/hiding author section
...
username("ProgrammerOwais") // defaul github user name
 ...
 submit.addEventListener('click',function() { // whenever github search submit button clicked
    if (input.value !== "") { // if input box contain value
...
function authorfunction(name) {
    ...
    fetch(`https://openlibrary.org/search/authors.json?q=${name}`).then(res=> { // Accessing author's data
        return res.json();
    }).then(data => {
        console.log(data);
        while (authorName.firstChild) {               // Clear all the contents of paragraph in html whenever authorfunction called.
        ...
        fetch(`http://covers.openlibrary.org/a/olid/${data.docs[0].key}-M.jpg`).then(res => { // fetching author image & displaying with text
            console.log(res);
...
authorSubmit.addEventListener('click',function() { //  By clicking author submit button
    if (authorInput.value!=="") { // if input contain value
...
photoBtn.addEventListener('click',function() {     // Initail button for showing/hiding photos API section
...
function randomAutoImgs() {
    fetch("https://api.pexels.com/v1/curated", { //Getting new images day by day & hour by hour
    method : "GET",  // methor for accessing somegthing
    headers : {     // we call it header for putting some private info i.e keys etc.
        "Authorization": "563492ad6f91700001000001f1f2fead15ad4340b3f9bd4209f797fb"
    },
}).then(res=> {
    return res.json();
}).then(data=> {
    while (pictureList.firstChild) { // Whenever randomAutoImgs() called clear all the childs in list i.e ul 
        pictureList.removeChild(pictureList.firstChild);
        
    }
    console.log("The pexel auto data is :",data);
    for (let i = 0; i < data.photos.length; i ++) { // Base on how many photos we have create elements
        ...
        pictureLink[i].textContent = "Visit to  "+ data.photos[i].photographer+ " web"; 
...
pictureSubmit.addEventListener('click', function() { // Whenever photo sumbit button clicked
    if (pictureSearch.value!=="") {                 // if input box is not empty
...
function photoSearch(name) {   // By using parameter 'name' you can access that paramenter type images.
...
function randomRecipes() {
    ...
    fetch('https://api.spoonacular.com/recipes/random?number=12&tags=vegetarian,dessert&apiKey=d3f492ca2e974370860d47cf0eb89c51',{
            method : "GET",
            headers : {
                ...
                return res.json()
            }).then(data=> { // This include recipes data every recipe enclosed in box
                ...
                for(let i = 0; i < data.recipes.length; i++) {
                    ...
                    recipeIconText[0].textContent ="Likes:" + data.recipes[i].aggregateLikes;
                    ...
                    recipeIconText[1].textContent ="$" + data.recipes[i].pricePerServing + "  per serving";
                    ...
                    recipeIconTextDiv[1].appendChild(recipeIconText[1]);
                     ...
                     recipeIconText[2].textContent = data.recipes[i].readyInMinutes + " mints";
                ...
                return data;
            }).then(data2=> { // This will include basic data of recipe i.e images , para , close btn etc.
                for(let i = 0;i < data2.recipes.length; i++) {
                    box[i].addEventListener('click',function(){ 
                        ...
                        recipe2CloseBtn.addEventListener('click', ()=> { //  Whenever closebtn cliked make the default behaviour of recipe as given below
                ...
                return data2;
            }).then(data2=>{ // This is data about recipe Nutrients.
                for(let i = 0;i < data2.recipes.length; i++) {
                    box[i].addEventListener('click',function(){
                        ...
                        fetch(`https://api.spoonacular.com/recipes/${data2.recipes[i].id}/nutritionWidget.json?apiKey=d3f492ca2e974370860d47cf0eb89c51`).then(res=>{
                            return res.json();
                        }).then(nutData=>{
            ...
            }).then(data2=> { // this is data about recipe ingredient
                for(let i = 0;i < data2.recipes.length; i++) {
                    box[i].addEventListener('click',function(){
                        ...
                        fetch(`https://api.spoonacular.com/recipes/${data2.recipes[i].id}/ingredientWidget.json?apiKey=d3f492ca2e974370860d47cf0eb89c51`).then(res=> {
                            return res.json()
                          }).then(ingreData=> {
                            ...
                            for(let j = 0; j<ingreData.ingredients.length; j++) {
                                ...
                                itemPara[j].textContent = `${ingreData.ingredients[j].amount.us.value} ${ingreData.ingredients[j].amount.us.unit} ${ingreData.ingredients[j].name}`;
                  ...
                  return data2 ;
            }).then(data2=> { // this is data about recipe related other links
                        for(let i = 0;i < data2.recipes.length; i++) {
                         box[i].addEventListener('click',function(){
...
function RecipeSearch(recipeName) {
    ...
    fetch(`https://api.spoonacular.com/recipes/complexSearch?query=${recipeName}&number=6&apiKey=d3f492ca2e974370860d47cf0eb89c51`,{
        method : "GET",
        headers : {
            'Content-Type':"application/json"
            }
                ...
                if (searchData.totalResults === 0 ) {
                    ...
                    for(let i = 0; i < searchData.results.length; i++) {
                        ...
                        fetch(`https://api.spoonacular.com/recipes/${searchData.results[i].id}/information?apiKey=d3f492ca2e974370860d47cf0eb89c51`).then(res => {
                            return res.json();
                        }).then(data=> { // This include recipes data every recipe enclosed in box
                                ...
                                recipeIconText[1] = document.createElement('span');
                                recipeIconText[1].classList.add('icon-text');
                                recipeIconText[1].textContent ="$" + data.pricePerServing + "  per serving";
                                recipeIconTextDiv[1].appendChild(recipeIcon[1]);
                                recipeIconTextDiv[1].appendChild(recipeIconText[1]);
                            ...
                            return data;
                                ...
                                box[i].addEventListener('click',function(){ 
                                    ...
                                    boxtop = box[i].offsetTop;         // Find the position of clicked box from top of root page
                                    recipeInfoStorage.style.top = `calc(${boxtop}px - 50px)` || boxtop +"px"; // show the recipe container in the area of selected box.
                                    recipeInfoStorage.style.left =" 50%";
                                    recipeInfoStorage.style.opacity ="1";
                            ...
                            return data2;
                                ...
                                box[i].addEventListener('click',function(){
                                    ...
                                    fetch(`https://api.spoonacular.com/recipes/${data2.id}/nutritionWidget.json?apiKey=d3f492ca2e974370860d47cf0eb89c51`).then(res=>{
                                        return res.json();
                                    }).then(nutData=>{
                                        console.log("the nut data is :", nutData);
                                            nutList[0] = document.createElement('span');
                            ...
                            return data2;
                                ...
                                box[i].addEventListener('click',function(){
                                    ...
                                    fetch(`https://api.spoonacular.com/recipes/${data2.id}/ingredientWidget.json?apiKey=d3f492ca2e974370860d47cf0eb89c51`).then(res=> {
                                        return res.json()
                                      }).then(ingreData=> {
                                        console.log("ingredient data is : ", ingreData);
                                        recipeInfoStorage.appendChild(subHeading2[i]);
